    ...
    def add_req(self, permission_item):
        ...
        for user_capability in permission_item.user_capabilities:
            ...
            grantee_capabilities_element = ET.SubElement(
                grantee_element, user_capability.User
            )
            ...
            self._add_capability(
                capabilities_element, user_capability.allowed, user_capability.Allow
            )
            self._add_capability(
                capabilities_element, user_capability.denied, user_capability.Deny
            )

        for group_capability in permission_item.group_capabilities:
            grantee_element = ET.SubElement(permissions_element, "granteeCapabilities")
            ET.SubElement(
                grantee_element, group_capability, id=group_capability.grantee_id
            )
            capabilities_element = ET.SubElement(grantee_element, "capabilities")
            self._add_capability(
                capabilities_element, group_capability.allowed, group_capability.Allow
            )
            self._add_capability(
                capabilities_element, group_capability.denied, group_capability.Deny
            )
        return ET.tostring(xml_request)
...
class SiteRequest(object):
    def update_req(self, site_item):
        ...
        if site_item.materialized_views_mode is not None:
            site_element.attrib["materializedViewsMode"] = str(
                site_item.materialized_views_mode
            ).lower()
        return ET.tostring(xml_request)
...
class WorkbookRequest(object):
    ...
    def update_req(self, workbook_item):
        ...
        if workbook_item.materialized_views_config is not None:
            ...
            materialized_views_element = ET.SubElement(
                workbook_element, "materializedViewsEnablementConfig"
            )
            materialized_views_element.attrib["materializedViewsEnabled"] = str(
                materialized_views_config["materialized_views_enabled"]
            ).lower()
            materialized_views_element.attrib["materializeNow"] = str(
                materialized_views_config["run_materialization_now"]
            ).lower()
