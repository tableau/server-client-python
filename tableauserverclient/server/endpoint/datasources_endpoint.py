...
from .. import RequestFactory, DatasourceItem, PaginationItem, \
               ConnectionItem, JobItem, Server, RequestOptions, PermissionsRule
...
class Datasources(QuerysetEndpoint):
    ...
    @api(version="2.0")
    def populate_connections(self, datasource_item: DatasourceItem):
    ...
    def _get_datasource_connections(self, datasource_item: DatasourceItem,
    ...
    def download(self, datasource_id: str, filepath: str = None,
                 include_extract: bool = True, no_extract: bool = None) -> str:
        if not datasource_id:
            error = "Datasource ID undefined."
            raise ValueError(error)
    ...
    @api(version='3.5')
    def delete_extract(self, datasource_item: DatasourceItem):
        id_ = getattr(datasource_item, 'id', datasource_item)
    ...
    def publish(self, datasource_item: DatasourceItem, file: str,
    ...
    @api(version='2.0')
    def populate_permissions(self, item: DatasourceItem):
        self._permissions.populate(item)

    @api(version='2.0')
    def update_permission(self, item: DatasourceItem, permission_item: PermissionsRule):
        import warnings
    ...
    @api(version='2.0')
    def update_permissions(self, item, permission_item: PermissionsRule):
        self._permissions.update(item, permission_item)
