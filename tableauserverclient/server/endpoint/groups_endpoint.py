...
class Groups(Endpoint):
    ...
    # Gets all groups
    @api(version="2.0")
    def get(self, req_options: RequestOptions = None) -> Tuple[List[GroupItem], PaginationItem]:
    ...
    @api(version="2.0")
    def populate_users(self, group_item: GroupItem,
    ...
    def _get_users_for_group(self, group_item: GroupItem, req_options: RequestOptions = None):
    ...
    @api(version="2.0")
    def create_AD_group(self, group_item: GroupItem,
                        asJob: bool = False) -> Union[JobItem, GroupItem]:
    ...
    @api(version="2.0")
    def remove_user(self, group_item: GroupItem, user_id: UserItem):
        if not group_item.id:
            error = "Group item missing ID."
            raise MissingRequiredFieldError(error)
        if not user_id:
    ...
    @api(version="2.0")
    def add_user(self, group_item: GroupItem, user_id: UserItem) -> UserItem:
        if not group_item.id:
            error = "Group item missing ID."
            raise MissingRequiredFieldError(error)
        if not user_id:
