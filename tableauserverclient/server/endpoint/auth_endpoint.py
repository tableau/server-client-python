...
class Auth(Endpoint):
    ...
    def sign_in(self, auth_req):
        auth_api = tsc_swagger.AuthenticationApi()
        auth_sign_in_req = tsc_swagger.SignInRequest(
            tsc_swagger.SignInRequestCredentials(
                name=auth_req.username,
                password=auth_req.password,
                site=tsc_swagger.SignInRequestCredentialsSite(auth_req.site_id),
                user=tsc_swagger.SignInRequestCredentialsUser(auth_req.user_id_to_impersonate)
            )
        )

        response = auth_api.auth_signin_post(auth_sign_in_req)
