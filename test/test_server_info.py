...
class ServerInfoTests(unittest.TestCase):
    ...
    def test_server_info_use_highest_version_downgrades(self):
        ...
        with requests_mock.mock() as m:
            ...
            m.get(
                self.server.server_address + "/api/2.4/serverInfo",
                text=si_response_xml,
                status_code=404,
            )
            m.get(
                self.server.server_address + "/auth?format=xml", text=auth_response_xml
            )
    ...
    def test_server_info_use_highest_version_upgrades(self):
        ...
        with requests_mock.mock() as m:
            m.get(
                self.server.server_address + "/api/2.4/serverInfo", text=si_response_xml
            )
            # Pretend we're old
