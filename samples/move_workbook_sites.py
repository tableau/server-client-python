...
source_site_server = TSA.Server(args.server)
dest_site_server = TSA.Server(args.server)

with source_site_server.auth.sign_in(tableau_auth):
    ...
    pagination_info, all_workbooks = source_site_server.workbooks.get(req_option)
    ...
    if len(all_workbooks) == 0:
        ...
        try:
            workbook_path = source_site_server.workbooks.download(all_workbooks[0].id, tmpdir)

            # Step 4: Sign in to destination site
            tableau_auth.site = args.destination_site
            with dest_site_server.auth.sign_in(tableau_auth):

                # Step 5: Find destination site's default project
                pagination_info, dest_projects = dest_site_server.projects.get()
                ...
                if target_project is not None:
                    ...
                    new_workbook = dest_site_server.workbooks.publish(new_workbook, workbook_path,
                                                                 mode=dest_site_server.PublishMode.Overwrite)
            ...
            source_site_server.workbooks.delete(all_workbooks[0].id)
        finally:
